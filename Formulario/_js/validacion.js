...
document.getElementById('Segundonombre').addEventListener('input', function () {
  ...
  const nombres = valor.split(' ').filter((nombre) => nombre.length > 0);
  if (valor.length < 3) {
    mostrarError('errorSegundonombre', 'El nombre debe tener al menos 3 caracteres');
    marcarCampo(this, false);
  } else {
    mostrarExito('exitoSegundonombre', '✓ Nombre válido');
    marcarCampo(this, true);
  }
});
...
document.getElementById('apellidos').addEventListener('input', function () {
  ...
  if (valor.length < 3) {
    ...
    mostrarError('errorApellidos', 'Debes ingresar al menos 2 apellidos');
    marcarCampo(this, false);
  } else {
    mostrarExito('exitoApellidos', '✓ Nombre válido');
    marcarCampo(this, true);
  }
});
...
document.getElementById('confirmarPassword').addEventListener('input', function () {
  ...
  if (this.value !== password || this.value.length === 0) {
    ...
    mostrarExito('exitoConfirmar', '✓ Coinciden');
    marcarCampo(this, true);
  }
});
